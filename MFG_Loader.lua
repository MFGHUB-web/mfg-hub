...
local S = {
	...
	isSellingNow           = false, -- Pauses auto-farm teleports while selling
	plantsSoldCount        = 0,
	
	-- Auto Clean Mutation (Washing Machine)
	autoCleanMutationEnabled = false,
	cleanMutations        = {},  -- e.g. {["Gold"] = true, ["Scorch"] = true} — mutation names to clean
	cleanMaxStars         = 2,   -- only clean mutations on plants with star tier <= this (2 stars and below)
	cleanMutationWorker   = false,
	cleansPerformedCount  = 0,
	claimedLaundryCount   = 0,
...
local R = {
	...
	GetLaundryState      = getRemote("GetLaundryStateRequest"),
...
-- ====================================================
local AUTH_API_URL   = "https://script.google.com/macros/s/AKfycbyrSDew8mAt2PqsxBgYsqtJNcawaQB6RNBjVqjg1viMPtpwHn-HPMfz8ydlpk3CL1x7/exec"  -- your Google Web App (validates username+key)
...
local function mfgVerifyKey(username, key)
	...
	pcall(function()
		...
		local body = ""
		if request then
			local res = request({ Url = url, Method = "GET", Timeout = 10 })
...
local function saveConfig(isManual)
	local cfg = {
		...
		autoCleanMutationEnabled = S.autoCleanMutationEnabled,
		cleanMutations          = S.cleanMutations,
		cleanMaxStars           = S.cleanMaxStars,
	}
...
local function findPlantableSeedTool()
	...
	local function scanContainer(container)
		...
		for _, t in ipairs(container:GetChildren()) do
			...
			-- Remember any seed tool as a fallback so a strict rarity filter
			-- can never abort planting entirely (intermittency fix).
			if not fallback and isSeedTool(t) then
				fallback = t
				fallbackR = PLANT_TO_RARITY[t.Name] or t:GetAttribute("Rarity") or "Common"
				if fallbackR == "" then fallbackR = "Common" end
			end
		end
		return nil
	end
	local m, n, r = scanContainer(char)
	if m then return m, n, r end
	m, n, r = scanContainer(bp)
	if m then return m, n, r end
	if fallback then return fallback, fallback.Name, fallbackR end
	return nil, nil, nil
end
...
task.spawn(function()
	while true do
		task.wait(1.5)
		if S.autoPlantSeedsEnabled and not S.farmPhaseActive then
...
local function isPlantUngrown(model)
	...
	-- 4) RolledSize ~= 1 means it has rolled a tier but is still growing
	--    (1 = fully grown / shovelable). Treat any not-yet-grown size as
	--    still in need of water, so attribute timing can't skip it.
	if model:GetAttribute("RolledSize") ~= 1 then
		return true
	end
	return false
end
...
task.spawn(function()
	while true do
		task.wait(4.5)
		if S.autoWaterEnabled and not S.farmPhaseActive then
...
local function runAutoSellPlantsCycle()
	...
	if #itemsToSell > 0 and root then
		-- Pause auto-farm teleports so the sell isn't torn down mid-sequence
		S.isSellingNow = true
		pcall(function()
			...
			for _, item in ipairs(itemsToSell) do
				if item.tool and item.tool.Parent then
					...
					if item.tool.Parent then
						-- The server refused / this one didn't go; don't count it
					else
...
-- ====================================================
--  🧼 AUTO CLEAN MUTATION (WASHING MACHINE)
--  Cleans a chosen mutation off plants that are Tier / star
--  <= S.cleanMaxStars (the mutation is kept on higher-tier plants).
--  Full flow: teleport to a free (Idle) machine -> equip the mutated
--  plant -> trigger its "Remove Mutation" prompt (costs sunshine) ->
--  wait until the machine reaches "ReadyToClaim" -> trigger "Claim"
--  to put the cleaned plant back in inventory.
-- ====================================================
local _laundryMachines = {}
local _laundryScannedAt = 0

local function collectLaundryMachines()
	if _laundryMachines and (tick() - _laundryScannedAt) < 15 then
		return _laundryMachines
	end
	local folder = workspace:FindFirstChild("WashingMachine")
	_laundryMachines = {}
	if folder then
		for _, m in ipairs(folder:GetChildren()) do
			if m:IsA("Model") then
				local laundry = m:FindFirstChild("Laundry")
				local prompt = laundry and laundry:FindFirstChildOfClass("ProximityPrompt")
				if laundry and prompt then
					table.insert(_laundryMachines, {
						model = m,
						laundry = laundry,
						prompt = prompt,
						machineId = tostring(m:GetAttribute("MachineId")),
					})
				end
			end
		end
	end
	_laundryScannedAt = tick()
	return _laundryMachines
end

-- Poll the server for the phase of each machine.
-- Returns a map { [machineId] = phase } and the raw list.
local function getLaundryPhases()
	local phases = {}
	if R.GetLaundryState then
		local ok, res = pcall(function() return R.GetLaundryState:InvokeServer() end)
		if ok and typeof(res) == "table" then
			for _, v in ipairs(res) do
				if v and v.machineId ~= nil then
					phases[tostring(v.machineId)] = v.phase or "Idle"
				end
			end
		end
	end
...
local function firePromptQuiet(prompt)
	if not prompt or not prompt.Enabled then return end
	if fireproximityprompt then
		pcall(function() fireproximityprompt(prompt, 0) end)
		pcall(function() fireproximityprompt(prompt) end)
...
-- Claim a machine currently showing the "Claim" prompt (ReadyToClaim).
local function claimLaundryMachine(machine, phases)
	local machineId = machine.machineId
	if not machine or not machine.prompt then return false end
	-- Only claim if the server reports it ReadyToClaim (or prompt says Claim)
	if phases and phases[machineId] and phases[machineId] ~= "ReadyToClaim" then
		return false
	end
	if machine.prompt.ActionText ~= "Claim" or not machine.prompt.Enabled then
		return false
	end
	local char = player.Character
	local root = char and char:FindFirstChild("HumanoidRootPart")
	...
	root.CFrame = CFrame.new(machine.laundry.Position + Vector3.new(0, 2, 0))
	task.wait(0.2)
	firePromptQuiet(machine.prompt)
	if orig then
		task.wait(0.2)
		root.CFrame = orig
	end
	S.claimedLaundryCount = S.claimedLaundryCount + 1
	return true
end

-- Insert an equipped mutated plant into an Idle machine, then wait + claim.
local function cleanOneMutatedPlant(tool)
	local root = player.Character and player.Character:FindFirstChild("HumanoidRootPart")
	if not root then return false end

	local machines = collectLaundryMachines()
	if #machines == 0 then return false end
	local phases = getLaundryPhases()

	-- 1) Claim anything already ReadyToClaim (from this or previous washes).
	local claimedAny = false
	for _, m in ipairs(machines) do
		if phases and phases[m.machineId] == "ReadyToClaim" then
			if claimLaundryMachine(m, phases) then claimedAny = true end
			task.wait(0.3)
		end
	end
	if claimedAny then task.wait(0.5) end
	phases = getLaundryPhases()

	-- 2) Find a free (Idle) machine to insert into.
	local target = nil
	for _, m in ipairs(machines) do
		if phases and phases[m.machineId] == "Idle" and m.prompt.ActionText ~= "Claim" then
			target = m
			break
		end
	end
	...
	-- 3) Equip the plant and fire the Remove-Mutation prompt on it.
	local okEquip = equipToolSafely(tool)
	task.wait(0.2)
	local orig = root.CFrame
	root.CFrame = CFrame.new(target.laundry.Position + Vector3.new(0, 2, 0))
	task.wait(0.2)
	if target.prompt.Enabled and target.prompt.ActionText ~= "Claim" then
		firePromptQuiet(target.prompt)
	end

	-- 4) Wait for this machine to become ReadyToClaim, then claim it.
	local targetId = target.machineId
	local waited = 0
	while waited < 200 do  -- up to ~200s
		task.wait(1)
		waited = waited + 1
		if not S.autoCleanMutationEnabled then break end
		phases = getLaundryPhases()
		if phases and phases[targetId] == "ReadyToClaim" then
			-- re-fetch its prompt; it may have been re-queried
			collectLaundryMachines()
			for _, m in ipairs(_laundryMachines) do
				if m.machineId == targetId then
					claimLaundryMachine(m, phases)
					break
				end
			end
			break
		end
		-- machine may have gone idle again (rejected / not inserted)
		if phases and phases[targetId] == "Idle" and waited > 2 then
			break
		end
	end
...
local function isMutationToClean(tool)
	if not tool then return false end
	local mutation = tool:GetAttribute("MutationType") or tool:GetAttribute("Mutation")
	if not mutation or mutation == "" or mutation == "None" then return false end
	if not S.cleanMutations[mutation] then return false end
	-- Star / tier rule: clean only if star count <= cleanMaxStars.
	-- MergeCount+1 == star count (0 merge = 1 star, 1 merge = 2 stars, ...).
	local mergeCount = safeNum(tool:GetAttribute("MergeCount") or tool:GetAttribute("StarLevel") or tool:GetAttribute("Star"), 0)
	local stars = mergeCount + 1
	return stars <= S.cleanMaxStars
end

local function runAutoCleanMutationCycle()
	if not S.autoCleanMutationEnabled then return end
	local bp = player:FindFirstChild("Backpack")
	local char = player.Character
	-- Find the first qualified mutated plant.
	local candidate = nil
	local function scanCont(cont)
		if not cont then return end
		for _, t in ipairs(cont:GetChildren()) do
			if isGrownPlantTool(t) and isMutationToClean(t) then
				candidate = t
				return
			end
		end
	...
	-- Guard against teleport fighting with the auto-farm pipeline.
	S.cleansPerformedCount = S.cleansPerformedCount + 1
	cleanOneMutatedPlant(candidate)
	if _G.MFGToast then
		_G.MFGToast("🧼 Mutation Cleaned", "Cleaned " .. candidate.Name, Color3.fromRGB(120, 220, 255))
	end
	return true
end
...
task.spawn(function()
	while true do
		task.wait(4)
		if S.autoCleanMutationEnabled and not S.farmPhaseActive then
			pcall(runAutoCleanMutationCycle)
		end
	end
end)

-- ====================================================
--  🎡 WHEEL AUTO-CLOSE (WHEN YOU LEAVE THE WHEEL)
--  The game's wheel stays open even after you walk away.
--  This closes it the moment you're far from the wheel so
--  it never stays stuck on screen. 25 studs = just past
--  the proximity prompt that opens it.
--  Also clears the fullscreen dim/blur overlay the game
--  leaves behind (UIPolishDimGui.DimOverlay) so the screen
--  is fully restored after the wheel closes.
-- ====================================================
task.spawn(function()
	while true do
		task.wait(1)
		local pg = player:FindFirstChild("PlayerGui")
		local wheelGui = pg and pg:FindFirstChild("SpinningWheel")
		if wheelGui and wheelGui.Enabled then
			local wp = workspace:FindFirstChild("ProximityParts")
				and workspace.ProximityParts:FindFirstChild("WheelPart")
			local root = player.Character and player.Character:FindFirstChild("HumanoidRootPart")
			if wp and root then
				if (root.Position - wp.Position).Magnitude > 25 then
					wheelGui.Enabled = false
					-- Clear the leftover dim / blur overlay
					local dimGui = pg and pg:FindFirstChild("UIPolishDimGui")
					if dimGui then
						local ov = dimGui:FindFirstChild("DimOverlay")
						if ov then ov.Visible = false end
						dimGui.Enabled = false
					end
					-- The game leaves a Lighting BlurEffect on when the wheel
					-- closes, which is the blur that stays on screen. Kill it.
					local lighting = game:FindFirstChild("Lighting")
					if lighting then
						local uiBlur = lighting:FindFirstChild("UIBlur")
						if uiBlur and uiBlur:IsA("BlurEffect") then uiBlur.Enabled = false end
					end
				end
			end
		end
...
local function claimFusionSeedOrResult()
	...
	if mergePart then
		...
		if mergePrompt and mergePrompt.Enabled and mergePrompt.ActionText == "Claim" then
			if root then root.CFrame = CFrame.new(mergePart.Position + Vector3.new(0, 1.5, 0)) end
			task.wait(0.3)
	...
	if resultPart then
		...
		if resultPrompt and resultPrompt.Enabled then
			if root then root.CFrame = CFrame.new(resultPart.Position + Vector3.new(0, 1.5, 0)) end
			task.wait(0.25)
	...
	if resultBin then
		...
		if binPrompt and binPrompt.Enabled then
			if root then root.CFrame = CFrame.new(resultBin.Position + Vector3.new(0, 1.5, 0)) end
			task.wait(0.25)
...
local function runAutoMergeCycle()
	...
	if not isMachineReadyForNewMerge() then
		return
	end
	...
	equipToolSafely(p1)
	task.wait(0.15)
	if root then root.CFrame = CFrame.new(fm.Plant1.Position + Vector3.new(0, 1.5, 0)) end
	task.wait(0.15)
	...
	equipToolSafely(p2)
	task.wait(0.15)
	if root then root.CFrame = CFrame.new(fm.Plant2.Position + Vector3.new(0, 1.5, 0)) end
	task.wait(0.15)
	...
	if root then root.CFrame = CFrame.new(fm.Merge.Position + Vector3.new(0, 1.5, 0)) end
	task.wait(0.15)
...
task.spawn(function()
	while true do
		task.wait(3)
		if S.autoShovelEnabled and not S.farmPhaseActive then
...
local function runAutoFarmPipeline()
	...
	if S.isSellingNow then return end -- let the sell cycle finish without teleport fights
	...
	pcall(function()
		...
		-- PHASE 0: INVENTORY FULL → MERGE FIRST
		-- When the backpack is stuffed with grown plants the shovel can't
		-- add more, so the farm stalls. Merge a pair first to free a slot
		-- so shoveling/planting can resume.
		local bp = player:FindFirstChild("Backpack")
		local grownInInv = 0
		if bp then
			for _, ti in ipairs(bp:GetChildren()) do
				if isGrownPlantTool(ti) then grownInInv = grownInInv + 1 end
			end
		end
		if grownInInv >= 40 then
			runAutoMergeCycle()
			task.wait(0.4)
		end
...
UI_Toggles.autoOpenPacks = createToggle(tabSeeds, "📦 Auto Open Seed Packs", "Continuously unpacks all seed packs in inventory (IsSeedPack tools)", S.autoOpenSeedsEnabled, function(val)
	S.autoOpenSeedsEnabled = val
	if val and R.OpenSeedPack and not S.autoOpenPacksWorker then
		S.autoOpenPacksWorker = true
		task.spawn(function()
			local opened = {}
			while S.autoOpenSeedsEnabled do
				-- Scan both character and backpack for seed-pack tools.
				local char = player.Character
				local containers = { char and char:FindFirstChild("Backpack"), player:FindFirstChild("Backpack") }
				local packs = {}
				for _, cont in ipairs(containers) do
					if cont then
						for _, t in ipairs(cont:GetChildren()) do
							if t:IsA("Tool") and t:GetAttribute("IsSeedPack") == true then
								table.insert(packs, t)
							end
						end
					end
				end
				...
				for _, pack in ipairs(packs) do
					if not S.autoOpenSeedsEnabled then break end
					if pack and pack.Parent then
						pcall(function() R.OpenSeedPack:FireServer(pack) end)
						S.seedPacksOpened = S.seedPacksOpened + 1
						opened[pack] = true
						if _G.MFGToast and (S.seedPacksOpened % 5 == 0) then
							_G.MFGToast("📦 Opened Seed Packs", tostring(S.seedPacksOpened) .. " opened!", Color3.fromRGB(200, 160, 255))
						end
					end
					task.wait(0.3)
				end
...
-- ====================================================
--  🧼 AUTO CLEAN MUTATION (WASHING MACHINE) UI
-- ====================================================
createSection(tabFusion, "Auto Clean Mutation (Washing Machine)")

UI_Toggles.autoCleanMutation = createToggle(tabFusion, "🧼 Auto Clean Mutation", "Washes the mutation off plants (tier <= max) via the Laundry machine, then auto-claims. Costs Sunshine per wash.", S.autoCleanMutationEnabled, function(val)
	S.autoCleanMutationEnabled = val
	if not val then return end
	if S.cleanMutationWorker then return end
	S.cleanMutationWorker = true
	task.spawn(function()
		-- Run continuously until disabled.
		while S.autoCleanMutationEnabled do
			if not S.farmPhaseActive then
				pcall(runAutoCleanMutationCycle)
			end
			task.wait(4)
		end
		S.cleanMutationWorker = false
	end)
end)

-- Mutation list input (comma / space separated mutation names to clean)
local cleanListCard = Instance.new("Frame")
cleanListCard.Size = UDim2.new(1, -6, 0, 96)
cleanListCard.BackgroundColor3 = C_CARD
cleanListCard.Parent = tabFusion
mkCorner(cleanListCard, 8)
mkStroke(cleanListCard, C_BORDER, 1)

local cleanLbl = Instance.new("TextLabel")
cleanLbl.Size = UDim2.new(1, -24, 0, 18)
cleanLbl.Position = UDim2.new(0, 12, 0, 6)
cleanLbl.BackgroundTransparency = 1
cleanLbl.TextColor3 = C_TEXT
cleanLbl.Font = Enum.Font.GothamBold
cleanLbl.TextSize = 12
cleanLbl.TextXAlignment = Enum.TextXAlignment.Left
cleanLbl.Text = "Mutations to Clean"
cleanLbl.Parent = cleanListCard

local cleanDesc = Instance.new("TextLabel")
cleanDesc.Size = UDim2.new(1, -24, 0, 14)
cleanDesc.Position = UDim2.new(0, 12, 0, 24)
cleanDesc.BackgroundTransparency = 1
cleanDesc.TextColor3 = C_SUBTEXT
cleanDesc.Font = Enum.Font.Gotham
cleanDesc.TextSize = 10
cleanDesc.TextXAlignment = Enum.TextXAlignment.Left
cleanDesc.Text = "Comma/space separated, e.g. Gold, Scorch, Bubble"
cleanDesc.Parent = cleanListCard

local cleanEdit = Instance.new("TextBox")
cleanEdit.Size = UDim2.new(1, -24, 0, 32)
cleanEdit.Position = UDim2.new(0, 12, 0, 44)
cleanEdit.BackgroundColor3 = Color3.fromRGB(13, 15, 22)
cleanEdit.BorderColor3 = Color3.fromRGB(60, 66, 94)
cleanEdit.PlaceholderText = "e.g. Gold, Scorch, Bubble"
cleanEdit.Font = Enum.Font.Gotham
cleanEdit.TextColor3 = Color3.fromRGB(230, 233, 245)
cleanEdit.TextSize = 13
cleanEdit.TextXAlignment = Enum.TextXAlignment.Left
-- Pre-fill from current state
local initialNames = {}
for n, on in pairs(S.cleanMutations) do if on then table.insert(initialNames, n) end end
table.sort(initialNames)
cleanEdit.Text = table.concat(initialNames, ", ")
cleanEdit.Parent = cleanListCard

local function applyCleanMutationsFromInput()
	local raw = cleanEdit.Text
	local nextList = {}
	for part in (raw .. ","):gmatch("([^,]+)") do
		local trim = part:gsub("%s+", "")
		if trim ~= "" then nextList[trim] = true end
	end
	for part in (raw .. " "):gmatch("([^ ]+)") do
		local trim = part:gsub("%,", ""):gsub("%s+", "")
		if trim ~= "" then nextList[trim] = true end
	end
	S.cleanMutations = nextList
	saveConfig(false)
	if _G.MFGToast then
		local c = 0
		for _ in pairs(nextList) do c = c + 1 end
		_G.MFGToast("🧼 Clean List Updated", tostring(c) .. " mutation(s) targeted", Color3.fromRGB(120, 220, 255))
	end
end

cleanEdit.FocusLost:Connect(function(enter)
	if enter then applyCleanMutationsFromInput() end
end)
UI_Toggles.cleanListApply = { apply = applyCleanMutationsFromInput }

createSection(tabFusion, "Clean Mutation Tier Limit")
UI_Toggles.cleanMaxStars = createToggle(tabFusion, "⭐ Only Clean 1-2 Star Plants", "Cleans the mutation only on plants up to 2 stars; 3+ star mutated plants are left untouched", S.cleanMaxStars <= 2, function(val)
	-- True = protect 3+ (clean <=2). False = clean every tier.
	S.cleanMaxStars = val and 2 or 99
	saveConfig(false)
end)

createActionButton(tabFusion, "Run One Clean Cycle", "Clean 1", Color3.fromRGB(35, 80, 140), function() task.spawn(runAutoCleanMutationCycle) end)
...
function loadConfig(silent)
	...
	if jsonOk and type(data) == "table" then
		...
		if data.autoCleanMutationEnabled ~= nil then S.autoCleanMutationEnabled = data.autoCleanMutationEnabled end
		if type(data.cleanMutations) == "table" then S.cleanMutations = data.cleanMutations end
		if data.cleanMaxStars ~= nil then S.cleanMaxStars = safeNum(data.cleanMaxStars, 2) end
		...
		if UI_Toggles.autoCleanMutation then UI_Toggles.autoCleanMutation.set(S.autoCleanMutationEnabled) end
		if UI_Toggles.cleanMaxStars then UI_Toggles.cleanMaxStars.set(S.cleanMaxStars <= 2) end
